...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="employees">{{ trans('cruds.project.fields.employees') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('employees') ? 'is-invalid' : '' }}" name="employees[]" id="employees" multiple required>
                    @foreach($employees as $id => $employees)
                        <option value="{{ $id }}" {{ (in_array($id, old('employees', [])) || $project->employees->contains($id)) ? 'selected' : '' }}>{{ $employees }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.employees_helper') }}</span>
            </div>
            <div class="form-group">
